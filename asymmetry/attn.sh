#!/bin/bash
# ==============================================================================
# Attention 服务启动脚本 (YAML配置版本)
# 用法: ./attn.sh [-d DEVICES] [-h]
# 示例: ./attn.sh -d 14,15
# ==============================================================================
...
# 解析命令行参数 (可覆盖YAML配置)
usage() {
    echo "用法: $0 [-d DEVICES] [-p PORT] [-a AFD_PORT] [-l LOG_DIR] [-m MASTER_ADDR] [-t MASTER_PORT] [-i INTERFACE] [-M MAX_MODEL_LEN] [-e EXPERT_PER_RANK] [-h]"
    echo "  参数:"
    echo "    -d DEVICES         使用的卡号（必填）"
    echo "    -p PORT            HTTP服务端口（默认: $ATTN_PORT）"
...
AFD_CONFIG='{
  ...
  "multistream_info": {"attn_enable": "False", "attn_core_num": "8", "ffn_enable": "True", "ffn_core_num": "8"},
  "afd_port": "'"$AFD_PORT"'"
}'
